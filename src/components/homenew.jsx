      ...
      if (formType === "1") {
        ...
        try {
          ...
          if (allRoots.some((root) => root.root > 0)) {
            throw new Error(
              "All roots must be non-positive (negative or zero only)"
            );
          }
          ...
          if (!isAlternatingRoots)
            throw new Error(
              "Circuit not feasible as poles and zeros do not alternate"
            );

          // Nearest root to origin
          const nearestRoot = combinedRoots.reduce((closest, current) => {
            return Math.abs(current.root) < Math.abs(closest.root)
              ? current
              : closest;
          });
          ...
          setError(
            err.message || "Calculation error occurred. Please try again."
          );
        } finally {
        ...
        let numCoeffs = generateCoefficients(
          numRoots.map((rootObj) => rootObj.root),
          numK
        );
        let denCoeffs = generateCoefficients(
          denRoots.map((rootObj) => rootObj.root),
          denK
        );
        ...
        // Nearest root to origin
        const nearestRoot = combinedRoots.reduce(
          (closest, current) => {
          ...
          { root: 0 }
        ); // Start with a default object where ro
      ...
      <div className="flex justify-center items-center pt-16">
        <div className="bg-white p-10 shadow-md rounded-lg w-full max-w-2xl">
          <p className="text-gray-600 mb-4">
            Enter the transfer function details below
          </p>
